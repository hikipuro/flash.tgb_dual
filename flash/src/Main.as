package 
{
	...
	import tgb_dual.CModule;
	import tgb_dual.getBytes;
	import tgb_dual.getSoundBytes;
	import tgb_dual.initTgbDual;
	import tgb_dual.loadRom;
	import tgb_dual.nextFrame;
	import tgb_dual.setKeys;
	
	/**
	 * TGB Dual の Flash 側のソース.
	 * @author Hikipuro
	 */
	public class Main extends Sprite
	{
		/** 初期表示サイズ. */
		private const _DEFAULT_SCALE:int = 4;
		
		/** TGB Dual 側から渡されたデータを描画するためのオブジェクト. */
		...
		private function _initTGBDual():void {
			...
			// 描画領域を準備
			_bitmapData = new BitmapData(160, 144);
			...
			// TGB Dual から受け取ったサウンドを再生するためのオブジェクトを初期化
		...
		private function _initUI():void {
			...
			// x4 ボタン
			var x4Button:PushButton = new PushButton(this, 250, 10, 'x4', function ():void {
				_setScale(4);
			});
			x4Button.width = 40;
			...
			// 使い方ラベル
			var howToLabel:Label = new Label(this, 370, 10, 'B: Z Key, A: X Key, Select: Shift Key, Start: Enter Key');
		}
		...
		private function _redrawBorder(scale:int):void {
			_background.graphics.clear();
			_background.graphics.lineStyle(1);
			_background.graphics.drawRect(0, 0, 160 * scale - 1, 144 * scale - 1);
		}
		...
		private function _onEnterFrame(e:Event):void {
			// TGB Dual の処理を 1 フレーム分進める
			nextFrame();
			
			// TGB Dual から画面のデータを 1 フレーム分受け取る
			var bytes:ByteArray = getBytes();
			bytes.endian = Endian.LITTLE_ENDIAN;
			bytes.position = 0;
			_bitmapData.setPixels(_bitmapData.rect, bytes);
			
			// キー情報を Flash から受け取って TGB Dual に渡す
			var down:int = 0;
			var up:int = 0;
			var left:int = 0;
			var right:int = 0;
