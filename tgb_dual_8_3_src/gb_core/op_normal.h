...
case 0x18: REG_PC+=(signed char)op_read()+1;break;//JR e : state 12

//JR cc,e : 00 1cc 000 : state 12(not jumped ->8)
case 0x20: if (REG_F&Z_FLAG) REG_PC+=1; else {REG_PC+=(signed char)op_read()+1;tmp_clocks=12;} break;// JRNZ
case 0x28: if (REG_F&Z_FLAG) {REG_PC+=(signed char)op_read()+1;tmp_clocks=12;} else REG_PC+=1; break;// JRZ
case 0x30: if (REG_F&C_FLAG) REG_PC+=1; else {REG_PC+=(signed char)op_read()+1;tmp_clocks=12;} break;// JRNC
case 0x38: if (REG_F&C_FLAG) {REG_PC+=(signed char)op_read()+1;tmp_clocks=12;} else REG_PC+=1; break;// JRC
